...
/*
  Снятие `Cross-Origin-Resource-Policy` с раздачи файлов.

  `secureHeaders()` ставит на все ответы `same-origin`, и для API это верно:
  чужая страница не должна тянуть из него данные. Но файлы — исключение:
  аватар подставляется в <img src> веб-панели, а она в разработке живёт на
  другом порту, и браузер отбивает запрос с
  ERR_BLOCKED_BY_RESPONSE.NotSameOrigin. Фото при этом загружено и ссылка
  рабочая — не видно только его. На проде панель и API на одном домене,
  поэтому дефект туда не доезжает и вылезает лишь на стенде.

  Стоит ВЫШЕ `secureHeaders`, и это не случайность: заголовки оба ставят
  после `await next()`, поэтому побеждает тот, кто зарегистрирован раньше —
  его код выполняется последним. Переставите ниже — заголовок снова
  затрётся на `same-origin`, причём молча.

  Ссылка на файл подписана и живёт недолго, так что снятие заголовка не
  открывает ничего сверх того, что открывает сама ссылка.
*/
