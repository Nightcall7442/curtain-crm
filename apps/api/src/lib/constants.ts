...
export const AUDIT_ACTIONS = [
  ...
  'fabric_stock.received',
  'fabric_stock.counted',
  'fabric_stock.written_off',

  'ready_made_item.created',
  'ready_made_item.updated',
...
/**
 * Что можно приложить к поручению.
 *
 * Шире, чем фото заказа: владелец просил «фото или что-то другое». Кроме
 * снимков это накладная или эскиз в PDF — то, что приносят на бумаге и
 * фотографируют либо сканируют.
 *
 * Список закрытый и здесь, а не «любой файл»: вложение попадает в хранилище
 * и потом отдаётся по подписанной ссылке, и принимать произвольный тип
 * значило бы раздавать со своего домена что угодно.
 */
export const ALLOWED_TASK_ATTACHMENT_MIME_TYPES = [
