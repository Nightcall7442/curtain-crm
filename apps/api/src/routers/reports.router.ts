    ...
    .query(async ({ ctx, input }) => {
      ...
      const [
        ...
        /*
          ...
          выбравший филиал, видел правильные заказы и число сотрудников по
          всей компании — расхождение внутри одной карточки.

          Сутки — не украшение условия. «Открыта» и «человек на работе» это
          разные вещи: смену забывают закрыть, и такая висит открытой
          неделями. Директор видел на главном экране четверых «на смене»,
          а в явке — одного: троих оставили открытыми в конце августа.
          Смена длиннее суток — это забытая отметка, а не работа.
        */
        ctx.db
          .select({ value: count() })
          .from(shifts)
