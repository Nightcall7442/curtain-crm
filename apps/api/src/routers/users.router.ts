 ...
 *  - `setAvatar`, `removeAvatar` — руководство (CEO, админ), по любому
 *    сотруднику: фото в карточке — рабочий документ компании, а не профиль
 *    в мессенджере, но снимает и загружает штат обычно администратор;
...
export const usersRouter = router({
  ...
  /**
   * Фото сотрудника ставит руководство — директор или администратор.
   *
   ...
   * кроме руководства — у швеи файла с её портретом просто нет.
   *
   * Сотрудник своё фото не меняет и не убирает: карточка в системе — рабочий
   * документ, а не профиль в мессенджере, и снимок в ней должен оставаться
   * тем, который поставила компания.
   *
   * Администратора владелец добавил отдельно: портреты загружает тот, кто
   * ведёт кадровые дела, и упираться в директора ради фотографии незачем.
   * Права при этом не размываются — права и роли по-прежнему только за ним.
   *
   * Каждая замена попадает в audit_log с именем того, кто её сделал: тихой
   * подмены не выйдет ни у администратора, ни у директора.
   */
  ...
  /** Убрать фото сотрудника: снова показываются инициалы. Тоже руководство. */
  removeAvatar: managementProcedure
