 ...
 * только тот, кто уже входил паролем с этого устройства, а выход из аккаунта
 * убирает его оттуда.
 *
    ...
    <BottomSheet visible={visible} title="Сохранённые входы" onClose={onClose}>
      {others.length === 0 ? (
        <Text style={styles.empty}>
          Других аккаунтов на этом телефоне нет. Войдите под нужным сотрудником
          паролем — и он появится здесь.
        </Text>
      ) : (
        others.map((account) => (
          <Pressable
