...
export const catalogItems = pgTable(
  'catalog_items',
  {
    ...
    /**
     * Короткое пояснение к значению — для справочников кодов материалов.
     *
     * Продавец переписывает с этикетки код («J-012034»), а что это за ткань,
     * знает только руководитель, который код завёл. Описание показывается
     * в форме заказа сразу после ввода кода и уезжает в позицию заказа
     * вместе с ним.
     *
     * У обычных справочников (модели, цвета) пустое: там название говорит
     * само за себя.
     */
    description: text('description'),

    /**
     * Для моделей штор — чем модель крепится: `pipe` или `cornice`
     * (`CURTAIN_MOUNT_KINDS` в `@curtain-crm/shared`).
     *
     * От этого зависит, какие строки материала форма заказа вообще покажет:
     * у «Трубы» и «Киприка» нет пластика с карнизом, у остальных моделей нет
     * трубы. Пустое значение читается как «пластик + карниз» — так стояли
     * все модели до появления групп.
     *
