...
export const readyMadeItems = pgTable(
  'ready_made_items',
  {
    ...
    */
    ...
    /**
     * Ключ комплекта: шторы с одним ключом лежат и продаются вместе.
     *
     * `order:<id>` — из пошива на склад, `manual:<id первой шторы>` —
     * заведённый руками (окно + дверь одной карточкой). Текст, а не ссылка:
     * у комплекта нет своей таблицы, и заводить её ради одной группировки
     * значило бы третью сущность там, где хватает подписи на двух.
     */
    setKey: text('set_key'),
  ...
  (table) => [
    index('ready_made_items_set_idx').on(table.setKey),
    /*
